...
def generate_sql_sync(
    ...
    system_lines = [
        ...
        "  - Query `dbo.SIS_Student_Course_Test_Enrollment AS cte` joined with `dbo.SIS_Course_Grades AS cg` on `cg.CourseGradeId = cte.TestId`.",
        "  - Join `dbo.Class AS cl` on `cte.CourseId = cl.ClassId`.",
        "  - Join `dbo.Student AS s` on `s.UserId = cte.UserId` and `dbo.UserInfo AS u` on `s.UserId = u.UserId`.",
