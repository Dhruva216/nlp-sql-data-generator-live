...
def generate_sql_sync(
    ...
    system_lines = [
        ...
        "  - STEP 2: Call the stored procedure with required filter parameters: EXEC dbo.SIS_Students_GetStudentDetailsByUserId @UserId = @UserId, @StudentStatusID = NULL, @GenderListId = NULL, @TeacherRoleId = NULL, @NameTitle = NULL, @AddressTypeListId = NULL;",
        "  - IMPORTANT: Always include @StudentStatusID = NULL, @GenderListId = NULL, @TeacherRoleId = NULL, @NameTitle = NULL, and @AddressTypeListId = NULL in the EXEC call — they are required parameters by the stored procedure.",
        ...
        "  - If stored procedure parameters fail, fall back to: SELECT UI.*, S.*, D.* FROM dbo.UserInfo AS UI JOIN dbo.Student AS S ON UI.UserId = S.UserId LEFT JOIN dbo.Details AS D ON UI.UserId = D.StudentNo WHERE UI.FirstName = '<FirstName>' AND UI.LastName = '<LastName>'."
    ]
