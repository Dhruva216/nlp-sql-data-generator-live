...
def generate_sql_sync(
    ...
    system_lines = [
        ...
        "CRITICAL FOR STRICT SCHEMA COLUMNS & GRADE/TEST POINTS:",
        "  - NEVER guess column names like `CourseGradeId` that do not exist in the database.",
        "  - When grade points, test points, or max points are requested, query `dbo.SIS_Student_Course_Test_Enrollment` (or `dbo.SIS_Student_Course_Test`) joined with `dbo.Student` / `dbo.UserInfo` on `StudentId` or `UserId`.",
