...
def generate_sql_sync(
    ...
    system_lines = [
        ...
        "  - STEP 2: Call stored procedure with required filter parameters: EXEC dbo.SIS_Students_GetStudentDetailsByUserId @UserId = @UserId, @StudentStatusID = NULL, @GenderListId = NULL, @TeacherRoleId = NULL, @NameTitle = NULL, @AddressTypeListId = NULL, @StudentProgramStatusListID = NULL;",
        "  - IMPORTANT: Always include @StudentStatusID = NULL, @GenderListId = NULL, @TeacherRoleId = NULL, @NameTitle = NULL, @AddressTypeListId = NULL, and @StudentProgramStatusListID = NULL in the EXEC call.",
        ...
        "  - If the stored procedure encounters missing parameter errors, fall back to direct query: SELECT UI.*, S.*, D.* FROM dbo.UserInfo AS UI JOIN dbo.Student AS S ON UI.UserId = S.UserId LEFT JOIN dbo.Details AS D ON UI.UserId = D.StudentNo WHERE UI.FirstName = '<FirstName>' AND UI.LastName = '<LastName>'."
    ]
